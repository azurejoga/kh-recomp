...
// One-time soft-cap warning: a legitimately spinning guest (IRQ/input wait)
// must not abort the process — the native loop would spin too.
static bool g_bridge_cap_warned = false;
...
extern "C" int runtime_bridge_interpret(uint32_t entry_pc, bool entry_thumb,
    ...
    for (;;) {
        ...
        if (++iters > cap) {
            ...
            gbarecomp::store_interp_into_arm_cpu(cpu, g_cpu);
            if (!g_bridge_cap_warned) {
                g_bridge_cap_warned = true;
                std::fprintf(stderr,
                    "runtime_arm: SELF-HEAL bridge for 0x%08X exceeded %llu "
                    "instructions without returning to stop_pc=0x%08X (current "
                    "pc=0x%08X). Guest is spinning legitimately (IRQ/input "
                    "wait); continuing without aborting.\n",
                    entry_pc,
                    static_cast<unsigned long long>(kBridgeIterationCap),
                    stop_pc, cpu.R[15]);
            }
            iters = 0;  // soft cap: keep interpreting (native loop semantics)
        }
    }
