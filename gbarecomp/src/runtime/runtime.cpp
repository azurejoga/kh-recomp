...
// Screen-reader mode (Prism). Toggled by HK_ACCESSIBILITY (Shift+A); the
// dialog/menu reader polls this each frame before speaking.
bool g_a11y_enabled = false;

int run_game(int argc, char** argv, const RunOptions& opts) {
    // Diagnostics for the Fase-2 screen reader (see a11y_dump.h). No-op
    // unless GBARECOMP_A11Y_DUMP is set.
    a11y_dump_start();
    ...
    auto pump_host_input = [&]() {
        ...
        if (ev.toggle_accessibility) {
            if (!a11y_active()) a11y_init();
            g_a11y_enabled = !g_a11y_enabled;
            if (g_a11y_enabled) {
                a11y_speak("Accessibility mode on", false);
                std::printf("accessibility=on\n");
            } else {
                std::printf("accessibility=off\n");
            }
            std::fflush(stdout);
        }
        if (ev.toggle_pause) {
    ...
    if (!args.dump_bmp.empty() || !args.dump_png.empty()) {
        ...
        a11y_dump_frame(bus.io().raw(), bus.vram_ptr());
