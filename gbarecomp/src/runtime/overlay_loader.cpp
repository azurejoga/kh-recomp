...
bool ram_overlay_heal_enabled() {
    // ON by default: the RAM-resident engines (audio/cutscene code the game
    // copies into EWRAM/IWRAM at runtime) get compiled to native overlays on
    // first hit instead of staying on the interpreter bridge — this is what
    // removes the dialogue-advance stutter. The bytes are snapshotted live
    // from guest RAM at the miss PC, so no static discovery is needed.
    // Disable with GBARECOMP_RAM_OVERLAY_HEAL=0 (or off/false) for a
    // pure-interpreter / oracle / cosim run.
    const char* value = std::getenv("GBARECOMP_RAM_OVERLAY_HEAL");
    if (!value || value[0] == '\0') return true;
    if (value[0] == '0' || std::strcmp(value, "off") == 0 ||
...
void worker_main() {
    for (;;) {
        ...
        try {
            ...
            r.ok = false;
            err  = std::string("worker exception: ") + ex.what();
        } catch (...) {
            r.ok = false;
            err  = "worker exception (unknown)";
        }
        if (r.ok && self_heal_verbose()) {
            std::fprintf(stderr,
                "self_heal: HEALED %s0x%08X (%s) -> native via %s (crc=%08X, "
...
int warm_load_cache_dir(const std::string& dir, HealBackend backend,
    ...
    for (const auto& de : fs::directory_iterator(dir, ec)) {
        ...
        try {
            ...
            std::fprintf(stderr,
                         "self_heal: warm-load exception for 0x%08X (%s): %s\n",
                         pc, thumb ? "thumb" : "arm", ex.what());
        } catch (...) {
            std::fprintf(stderr,
                         "self_heal: warm-load exception for 0x%08X (%s)\n",
                         pc, thumb ? "thumb" : "arm");
        }
    }
    return loaded;
...
bool overlay_request_compile(uint32_t pc, bool thumb) {
    ...
    if (sync_overlay_heal_enabled()) {
        OverlayCompiled c;
        std::string err;
        bool ok = false;
        try {
            ok = overlay_compile_one(w, s_active_cache_dir, &g_callbacks,
                                     /*compile_if_missing=*/true, s_backend,
                                     &c, &err);
        } catch (const std::exception& ex) {
            err = std::string("sync compile exception: ") + ex.what();
        } catch (...) {
            err = "sync compile exception (unknown)";
        }
        if (ok) {
            install_healed(key, c);
