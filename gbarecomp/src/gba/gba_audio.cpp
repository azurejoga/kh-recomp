...
void GbaAudio::write_io8(uint32_t off, uint8_t v) {
    ...
    switch (off) {
        ...
        case 0x084: {
            sample_until_current_time();
            bool was_master = master_enable_;
            master_enable_ = (v & 0x80) != 0;
            if (was_master != master_enable_) {
                std::fprintf(stderr,
                             "[audio] SOUNDCNT_X=0x%02x master=%d (prev=%d)\n",
                             v, master_enable_ ? 1 : 0, was_master ? 1 : 0);
                std::fflush(stderr);
            }
            if (!master_enable_) {
...
std::size_t GbaAudio::drain_samples(int16_t* out, std::size_t max) {
    ...
    while (n < max && ring_tail_ != ring_head_) {
        int16_t s = ring_[ring_tail_];
        out[n++] = s;
        if (s != 0) ++nz;
        ring_tail_ = (ring_tail_ + 1) % kRingSize;
    }
    if (n > 0) {
        // No per-frame debug spam by DEFAULT (stalls the game writing to a
        // log on every frame). Only emit when GBARECOMP_DEBUG_AUDIO is set.
        static const bool& audio_debug_on = []() -> const bool {
            static const bool on =
                std::getenv("GBARECOMP_DEBUG_AUDIO") && *std::getenv("GBARECOMP_DEBUG_AUDIO") != '0';
            return on;
        }();
        if (audio_debug_on) {
            ...
            if ((++log_cnt & 0x3Fu) == 1u) {
                std::fprintf(stderr,
                             "[audio] drain n=%zu nonzero=%zu master=%d volL=%d volR=%d "
                             "dA_l=%d dA_r=%d dB_l=%d dB_r=%d ch1=%d ch2=%d ch3=%d ch4=%d "
                             "fifoA_count=%u fifoA_s0=%d fifoB_count=%u fifoB_s0=%d "
                             "shadow_en=%d\n",
                             n, nz, master_enable_ ? 1 : 0, volume_l_, volume_r_,
                             direct_a_left_ ? 1 : 0, direct_a_right_ ? 1 : 0,
                             direct_b_left_ ? 1 : 0, direct_b_right_ ? 1 : 0,
                             ch1_.active ? 1 : 0, ch2_.active ? 1 : 0,
                             ch3_.active ? 1 : 0, ch4_.active ? 1 : 0,
                             fifo_a_.count, fifo_a_.samples[0],
                             fifo_b_.count, fifo_b_.samples[0],
                             shadow_enabled_ ? 1 : 0);
            }
        }
    }
