...
enum HostHotkey {
    ...
    // Screen-reader toggle (Shift+A). Speaks on/off through Prism.
    HK_ACCESSIBILITY,
    HK_COUNT
};
...
void gba_audio_callback(void* userdata, Uint8* stream, int len) {
    ...
    // GBARECOMP_AUDIO_DUMP=<path.wav> — write raw mono S16 65536 Hz samples.
    static FILE* dump = nullptr;
    static long total = 0;
    static constexpr long kMax = 65536L * 2L * 120;  // bytes for up to 120 s (mono S16)
    if (!dump) {
        if (const char* e = std::getenv("GBARECOMP_AUDIO_DUMP"); e && *e) {
            dump = fopen(e, "wb");
            if (dump) {
                ...
                unsigned r = 65536u;
                memcpy(hdr + 24, &r, 4);          // sample rate
                unsigned br = 65536u * 2u;
                memcpy(hdr + 28, &br, 4);         // byte rate
                hdr[32] = 2; hdr[33] = 0;         // block align
                hdr[34] = 16; hdr[35] = 0;        // bits per sample
                hdr[36] = 'd'; hdr[37] = 'a'; hdr[38] = 't'; hdr[39] = 'a';
                fwrite(hdr, 1, 44, dump);
            }
        }
    }
    if (dump && total < kMax) {
        int n = len < (kMax - total) ? len : static_cast<int>(kMax - total);
        fwrite(stream, 1, n, dump);
        fflush(dump);  // survive unclean shutdown (timeout kill / thrown exit)
        total += n;
        if (total >= kMax) {
            fclose(dump);
            dump = nullptr;
...
bool HostWindow::open(int scale, int base_w, int base_h, const char* title,
    ...
    SDL_SetHintWithPriority(SDL_HINT_RENDER_DRIVER, "direct3d11",
                            SDL_HINT_DEFAULT);
#endif
    // Vsync OFF is the DEFAULT (fast, no present stall); the independent
    // FramePacer still governs emulation at the GBA's 59.7275 Hz, so the game
    // does not run turbo. Only GBARECOMP_VSYNC=1 enables sync-to-scanout.
    const char* vsync_env = std::getenv("GBARECOMP_VSYNC");
    const bool want_vsync =
        vsync_env && *vsync_env && vsync_env[0] != '0';
    ...
    // Direct (native) audio is the DEFAULT: it queues 65536Hz mono straight
    // to the device with no guest-clock bridge (no stutter when advancing
    // dialogue). GBARECOMP_AUDIO_DIRECT=0 opts out to the callback bridge.
    const char* direct_audio_env = std::getenv("GBARECOMP_AUDIO_DIRECT");
    b->audio_direct = true;  // native by default (embedded product default)
    if (direct_audio_env && *direct_audio_env) {
        // env fully overrides: "0" disables native, anything else enables
        b->audio_direct = (*direct_audio_env != '0');
    }
...
// Map a SDL scancode to a Windows virtual-key code, for the global
// GetAsyncKeyState fallback (input works even when this window lacks
// keyboard focus, e.g. under a screen reader). Returns 0 for scancodes
// without a sensible VK equivalent.
static int scancode_to_vk(SDL_Scancode sc) {
    switch (sc) {
        case SDL_SCANCODE_RETURN: return VK_RETURN;
        case SDL_SCANCODE_RETURN2: return VK_RETURN;
            ...
            if (sym == SDLK_ESCAPE) {
                ...
                for (int h = 0; h < HK_COUNT; ++h) {
                    ...
                    switch (h) {
                        ...
                        case HK_ACCESSIBILITY:  ev.toggle_accessibility = true; break;
                    }
                }
            }
        }
...
// Build the GBA KEYINPUT value from current keyboard state via the
    ...
    for (int bit = 0; bit < 10; ++bit) {
        ...
        // Global keyboard fallback (Windows): SDL_GetKeyboardState only
        // reports keys while THIS window has SDL keyboard focus, which is
        // never the case when a screen reader (NVDA/JAWS) or another app
        // holds focus. GetAsyncKeyState reads the real OS key state
        // regardless of focus, so the game always receives input. This is
        // what makes the intro skippable and dialogues advance for
        // keyboard-only / assistive-tech players.
        int vk = scancode_to_vk(sc);
