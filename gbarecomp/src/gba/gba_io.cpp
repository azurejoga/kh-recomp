...
void GbaIo::run_sound_fifo_dma(int channel) {
    ...
    // No per-frame debug spam by DEFAULT (it stalls the game writing to a
    // log on every frame). Only emit when GBARECOMP_DEBUG_DMA is set.
    static const bool& dma_debug_on = []() -> const bool {
        static const bool on =
            std::getenv("GBARECOMP_DEBUG_DMA") && *std::getenv("GBARECOMP_DEBUG_DMA") != '0';
        return on;
    }();
    if (dma_debug_on) {
        static uint32_t fifo_dma_log = 0;
        if ((++fifo_dma_log & 0x3Fu) == 1u) {
            std::fprintf(stderr, "[audio] fifo_dma ch=%d sad=0x%08x dad=0x%08x\n",
                         channel,
                         load_u32(&io_[base + 0]) & 0x0FFFFFFFu,
                         load_u32(&io_[base + 4]));
            std::fflush(stderr);
        }
    }
